...
function generateOutputTypeClassFromType(project, dirPath, type, dmmfDocument) {
    ...
    imports_1.generatePrismaNamespaceImport(sourceFile, dmmfDocument.options, 2);
    imports_1.generateCustomScalarsImport(sourceFile, 2);
    imports_1.generateArgsImports(sourceFile, fieldArgsTypeNames, 0);
    imports_1.generateOutputsImports(sourceFile, type.fields
...
function generateInputTypeClassFromType(project, dirPath, inputType, _dmmfDocument, options) {
    ...
    imports_1.generateTypeGraphQLImport(sourceFile);
    imports_1.generateGraphQLScalarsImport(sourceFile);
    imports_1.generatePrismaNamespaceImport(sourceFile, options, 2);
    imports_1.generateCustomScalarsImport(sourceFile, 2);
    imports_1.generateInputsImports(sourceFile, inputType.fields
    ...
    imports_1.generateEnumsImports(sourceFile, inputType.fields
    ...
    const omittedModelFields = _dmmfDocument.datamodel.models
        ...
        .flat();
    const inputFields = inputType.fields.filter(field => !omittedModelFields.includes(field.name));
    ...
    sourceFile.addClass({
        ...
        properties: inputFields.map(field => {
            return {
                name: field.name,
                type: field.fieldTSType,
