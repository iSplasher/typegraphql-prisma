"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
const tslib_1 = require("tslib");
const path_1 = tslib_1.__importDefault(require("path"));
const config_1 = require("../config");
const imports_1 = require("../imports");
const helpers_1 = require("./helpers");
const types_1 = require("../dmmf/types");
function generateActionResolverClass(project, baseDirPath, model, action, mapping, dmmfDocument) {
    const sourceFile = project.createSourceFile(path_1.default.resolve(baseDirPath, config_1.resolversFolderName, config_1.crudResolversFolderName, model.typeName, `${action.actionResolverName}.ts`), undefined, { overwrite: true });
    imports_1.generateTypeGraphQLImport(sourceFile);
    if (action.kind === types_1.DMMF.ModelAction.aggregate ||
        action.kind === types_1.DMMF.ModelAction.groupBy) {
        imports_1.generateGraphQLFieldsImport(sourceFile);
    }
    if (action.argsTypeName) {
        imports_1.generateArgsImports(sourceFile, [action.argsTypeName], 0);
    }
    imports_1.generateModelsImports(sourceFile, [model.name, action.outputTypeName]
        .filter(typeName => dmmfDocument.isModelName(typeName))
        .map(typeName => dmmfDocument.getModelTypeName(typeName)), 3);
    imports_1.generateOutputsImports(sourceFile, [action.outputTypeName].filter(typeName => !dmmfDocument.isModelName(typeName)), 2);
    imports_1.generateHelpersFileImport(sourceFile, 3);
    sourceFile.addClass({
        name: action.actionResolverName,
        isExported: true,
        decorators: [
            {
                name: "TypeGraphQL.Resolver",
                arguments: [`_of => ${model.typeName}`],
            },
        ],
        methods: [
            helpers_1.generateCrudResolverClassMethodDeclaration(action, mapping, dmmfDocument),
        ],
    });
}
exports.default = generateActionResolverClass;
//# sourceMappingURL=separate-action.js.map